...
class maxConnect4Game:
    ...
    def aiPlay(self, depth):
        randColumn = MiniMaxAlphaBeta(self, depth, 1)
...
def isValidMove(gameBoard, column):
...
def tryPiece(gameBoard, column, currentTurn):
    if not gameBoard[0][column]:
        for i in range(5, -1, -1):
            if not gameBoard[i][column]:
...
def MiniMaxAlphaBeta(currentGame, depth, player):
    pieceCount = 0 
    ...
    for move in validMoves:
        temp = copy.deepcopy(currentGame.gameBoard)
        tryPiece(temp, move, player)
        pieceCount += 1
        player, opponent = opponent, player
        boardScore = minimizeBeta(temp, depth - 1, alpha, beta, player, opponent, pieceCount)
...
def minimizeBeta(game, depth, a, b, player, opponent, pieceCount):
    validMoves = []
    for col in range(7):
        tempGame = game
        if tryPiece(tempGame, col, player):
            validMoves.append(col)

    # check to see if game over
    if depth == 0 or len(validMoves) == 0 or pieceCount == 42:
        return countScore(game)[0]
    ...
    for move in validMoves:
        ...
        if a < beta:
            ...
            tryPiece(game, move, player)
            pieceCount += 1
            player, opponent = opponent, player
            boardScore = maximizeAlpha(temp, depth - 1, a, beta, player, opponent, pieceCount)
...
def maximizeAlpha(game, depth, a, b, player, opponent, pieceCount):
    validMoves = []
    for col in range(7):
        tempGame = game
        if tryPiece(tempGame, col, player):
    ...
    for move in validMoves:
        ...
        if alpha < b:
            ...
            pieceCount += 1
            player, opponent = opponent, player
            boardScore = minimizeBeta(temp, depth - 1, alpha, b, player, opponent, pieceCount)
